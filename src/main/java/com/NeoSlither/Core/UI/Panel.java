...
public class Panel {
    ...
    public static boolean paused = false; // Variabile per tracciare lo stato di pausa
    ...
    public void update() {
        // Controlla se il gioco è in pausa o finito
        if (gameOver) {
            return; // Non aggiornare il gioco se è finito
        }

        // Cambia lo stato di pausa se viene premuto il tasto spazio
        if (InputHandler.spacePressed) {
            paused = !paused; // Inverte lo stato di pausa
            InputHandler.spacePressed = false; // Reset del flag per evitare input multipli
        }

        if (paused) {
            return; // Non aggiornare il gioco se è in pausa
        }
    ...
    public void draw(Graphics2D g2) {
        ...
        // Disegna "Paused" se il gioco è in pausa
        if (paused) {
            x = left_x;
            y = top_y + 320;
